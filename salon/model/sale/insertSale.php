			...
			if($stockStatement->rowCount() > 0){
				...
				if($currentQuantityInItemsTable <= 0) {
					...
					if($customerStatement->rowCount() > 0){
						...
						$insertSaleSql = 'INSERT INTO sale(itemName, discount, quantity, unitPrice, customerName, saleDate) VALUES( :itemName, :discount, :quantity, :unitPrice, :customerName, :saleDate)';
						$insertSaleStatement = $conn->prepare($insertSaleSql);
						$insertSaleStatement->execute(['itemName' => $itemName, 'discount' => $discount, 'quantity' => $quantity, 'unitPrice' => $unitPrice, 'customerName' => $customerName, 'saleDate' => $saleDate]);
						...
						echo '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">&times;</button>Sale details added successfully</div>';
						exit();
