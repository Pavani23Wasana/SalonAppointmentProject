...
foreach($result as $row)
{
	...
	$sub_array[] = '<a href="view_order.php?pdf=1&order_id='.$row["inventory_order_id"].'" class="btn btn-info btn-xs">View Bill</a>';
	$sub_array[] = '<button type="button" name="update" id="'.$row["inventory_order_id"].'" class="btn btn-primary btn-xs update">Update</button>';
	$sub_array[] = '<button type="button" name="delete" id="'.$row["inventory_order_id"].'" class="btn btn-success btn-xs delete" data-status="'.$row["inventory_order_status"].'">Change Status</button>';
	$data[] = $sub_array;
}
